...
def run_autofs(cfg: AutoFSConfig, progress_cb=None) -> Path:
    autodiscover()
    ...
    def _prog(p: float, msg: str):
        ...
        try:
            ...
            # Progress callback should never break execution.
            return

    start = time.time()
    _prog(0.02, "Loading dataset")
    ...
    _prog(0.10, "Preprocessing: fit + transform")
    ...
    for pi, phase in enumerate(cfg.phases):
        _prog(0.15 + 0.45 * (pi / n_phases), f"Running {phase.name}")
        rank_frames: List[pd.DataFrame] = []

        n_methods = max(len(phase.methods), 1)
        for mi, m in enumerate(phase.methods):
            ...
            # Fine-grained progress (method level)
            phase_base = 0.15 + 0.45 * (pi / n_phases)
            phase_span = 0.45 / n_phases
            meth_p = phase_base + phase_span * (mi / n_methods)
            _prog(min(meth_p, 0.65), f"{phase.name}: {REGISTRY.resolve(name)}")
            ...
            # Method-level progress update
            _prog(
                0.15 + 0.45 * (pi / n_phases) + 0.45 * (1 / n_phases) * ((mi + 1) / n_methods),
                f"{phase.name}: {REGISTRY.resolve(name)}",
            )
    ...
    if cfg.stability.enabled:
        _prog(0.70, "Stability selection")
    ...
    if cfg.redundancy.enabled:
        _prog(0.78, "Redundancy: correlation clusters")
    ...
    _prog(0.86, "Aggregating results")
